...
- Scorrere la lista e cliccare sul proprio indirizzo di email
  ![joinClassroom](./img/NostraImmagine3.png)
...
- Aspettare che GitHub Classroom cloni il repository base per te.
  ![configuringRepository](./img/NostraImmagine10.png)

- *Fai un refresh della pagina web per verificare il completamento.
  ![readyToGo](./img/NostraImmagine2.png)
...
- Scorrere la lista e cliccare sul proprio indirizzo di email
  ![joinClassroom](./img/NostraImmagine9.png)

- Unisciti al tuo team cliccando il corrispondente pulsante `Join`
   ![joinTeam](./img/NostraImmagine12.png)
...
- Se ti sei unito a un team già creato, dovrai prima confermare la scelta e quindi avrai subito la conferma.
   ![promptAcceptAssginment](./img/NostraImmagine11.png)
...
2. l'analisi dello stesso con strumento di quality assurance [Ruff](https://astral.sh/ruff);
...
- recarsi sulla pagina principale del repository e fare click sull'icona *"Settings"* (ultima tab in alto a destra);
  **N.B.**: solo l'amministratore visualizza questa tab!
  ![SaveTokenInSecret_5](./img/NostraImmagine1.png)
...
(vedi freccia rossa in figura). ![ExecuteDockerImage_1](./img/NostraImmagine8.png)
...
1. Fare click sul link al package per entrare nella pagina dedicata, quindi cliccare sul link "Package settings", presente nella barra laterale destra (vedi freccia rossa in figura). ![ExecuteDockerImage_1](./img/NostraImmagine7.png)
...
![Update_GitHub_badge_1](./img/NostraImmagine5.png)
...
- Lasciando invariate le impostazioni di default (`branch` e `event`) nella finestra a comparsa, fare click su `Copy status badge Markdown`;
  ![Update_GitHub_badge_3](./img/NostraImmagine4.png)
